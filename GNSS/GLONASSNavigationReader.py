...
class GLONASSNavigationReader(object):
    '''!
    classdocs
    '''


    def __init__(self, GLONASSNavigationFile):
        '''!
        Constructor
        '''
        #Comprobación del fichero.
        if not type(GLONASSNavigationFile)==str:
            raise Exception("La ruta del fichero introducido no es de tipo str.")
        if not exists(GLONASSNavigationFile):
            raise Exception("La ruta del fichero introducido no existe.")
        if not isfile(GLONASSNavigationFile):
            raise Exception("La ruta del fichero introducido no corresponde a ningun fichero.")
        
        if GLONASSNavigationFile.endswith("g") or GLONASSNavigationFile.endswith("G") and search("\d\d",GLONASSNavigationFile.split(".")[1]):
                pass
        else:
        ...
        f=open(GLONASSNavigationFile,'r')
        for i in f:
    ...
    def getHeader(self):
        '''!
        @brief: Función que devuelve los parametros informativos de la cabecera del fichero de nvegación GLONASS
        ...
        for i in self.head:
            ...
            if "DELTA-UTC: A0,A1,T,W" in i[60:80]:
                ...
                continue
            if "CORR TO SYSTEM TIME" in i[60:80]:
                sal.update({"CORRECTIOn TIME":float(" ".join(i[22:40].split()).replace("D","E"))})
...
##    print(rutasample)
    ...
    print(ficheros)
    val=[i for i in ficheros if i.endswith("g") or i.endswith("G")]
##    print(val)
    seleccionado=None
    ...
    nav=GLONASSNavigationReader(rutadescom)
##    cab=nav.get_Header()
